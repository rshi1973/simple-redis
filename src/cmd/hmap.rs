...
impl TryFrom<RespArray> for HMGet {
    type Error = CommandError;
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        // The number args is at least 2
        validate_command(&value, &["hmget"], 2)?;

        //extract the first arg as key
        let mut args = extract_args(value, 1)?.into_iter();
        ...
        //extract the rest of the args as fields
        let fields = args
            .map(|arg| match arg {
                RespFrame::BulkString(field) => Ok(String::from_utf8(field.0)?),
                _ => Err::<String, _>(CommandError::InvalidArgument("Invalid field".to_string())),
            })
            .collect::<Result<Vec<String>, CommandError>>()?;

        //return the result of HMGet
        Ok(HMGet { key, fields })
    }
}
